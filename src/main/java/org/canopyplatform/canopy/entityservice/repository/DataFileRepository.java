...
public interface DataFileRepository extends JpaRepository<DataFile, Integer> {
    ...
    @Query(value = "SELECT center as name, " +
            ...
            "GROUP BY center",
            nativeQuery = true)
    List<Map<String, Object>> getCenterStats();
    ...
    @Query(value = "select ds.study_id, vs.title, vs.center, count(distinct df.id) as files, ds.date_approved as date " +
            ...
            "group by ds.study_id, vs.title, vs.center, ds.date_approved order by ds.date_approved desc", nativeQuery = true)
    ...
    @Query(value = "select ds.study_id, vs.title, vs.center, count(distinct df.id) as files, ds.date_approved as date " +
            ...
            "group by ds.study_id, vs.title, vs.center, ds.date_approved " +
